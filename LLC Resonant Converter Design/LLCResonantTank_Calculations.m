...
fr = 75e3; % Resonant frequency in Hz
Vin_nom = 23; % Nominal input voltage in V
Vout = 22.2; % Output voltage in V
Nps = (Vin_nom / (2*Vout))/0.5; % Transformer turns ratio Half Bridge 
% for Full Bridge Nps = (Vin_nom / Vout);
Pout_max = 300; % Maximum output power in W
m = 6; % Ratio (Lr + Lm) / Lr
Q_max = 1.2; % Maximum quality factor
...
Q_values = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2];
